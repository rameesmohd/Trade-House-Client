...
const ModulesList = () => {
  ...
  const AddChapterhandle = ({id,save,cancel}) => {
    ...
    if(save){
      ...
      axiosInstance.post('/add-chapter',{moduleId:id,title:title,courseId:courseId}).then((res)=>{
  ...
  const DropdownAndUpHandler=({id,dropupIcon})=>{
      setDrop({_id:id,DropupIcon : dropupIcon})
  }
  ...
  const editChapterHandle=(e)=>{
      ...
      setSpinner(true)
      axiosInstance.post('/update-chapter',{title:title,video:video ? video : currVideo,moduleId:currentModuleId,chapterId:chapterId},{
  ...
  return (
    ...
    <div className='lg:grid lg:grid-cols-2 w-full h-full py-2'>
          ...
          return <div className=" overflow-x-auto shadow-md sm:rounded-lg mb-5">
              ...
              <tbody>
                  <tr  className="bg-white border-b">
                    ...
                    <td className="px-6 py-4">
                      ...
                      { obj?.chapters?.length ? !drop.DropupIcon &&
                      <td onClick={()=>DropdownAndUpHandler({id:obj._id,dropupIcon :true})} className="px-6 py-4 flex cursor-pointer">
                          <a>Chapters</a>
                        <DropdownIcon/>
                      </td> : ''}
                      { obj?.chapters?.length ? drop.DropupIcon &&
                      <td onClick={()=>DropdownAndUpHandler({id:'',dropupIcon :false})} className="px-6 py-4 flex cursor-pointer">
                        <a>Chapters</a>
                        <DropupIcon/>
                      </td> : ''}
                    </tr>
            ...
            <div className='w-full py-3 px-4'>
              ...
              {drop._id==obj._id && obj?.chapters?.map((chapter,index)=>
                ...
                <button onClick={()=>expandChapter(chapter,obj._id)} className='w-full m-1 border flex justify-between border-black rounded-md h-auto p-3 focus:bg-slate-500 focus:text-white text-left ' >
      ...
      <form className='lg:col-span-1 h-fit w-full bg-slate-50 flex justify-center' onSubmit={editChapterHandle} encType="multipart/form-data">
            ...
            { !editChapter && <h1 className='text-lg font-poppins'>Title : {chapterdata.chapter_title}</h1>}
             { editChapter && <div className='flex'>
              ...
              <div className='flex justify-end'>
                ...
                { editChapter && <div className='flex justify-between m-1'>
                  <div className='btn-sm flex justify-center items-center text-white rounded-md h-7 bg-red-700 cursor-pointer'>Cancel</div>
                </div>}
                { !editChapter && <div className='flex justify-between m-1'>
                  <div className='btn-sm flex justify-center items-center text-white rounded-md h-7 bg-red-700 cursor-pointer'>Delete</div>
                </div>}
              </div>
            </div>
          </div>
